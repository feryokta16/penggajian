                <!-- Begin Page Content -->
                <div class="container-fluid">

                    <!-- Page Heading -->
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800"><?php echo $title ?></h1>
                    </div>
                    <div class="card" style="width:60%">
                        <div class="card-body">
                            <?php  foreach ($jabatan as $j): ?>
                        <form action="<?php echo base_url('admin/dataJabatan/updateDataAksi')?>" method="POST">
                        <div class="form-group">
                            <label>Nama Jabatan</label>
                            <input type="hidden" name="id_jabatan" class="form-control" value="<?php echo $j->id_jabatan?>">
                            <input type="text" name="nama_jabatan" class="form-control" value="<?php echo $j->nama_jabatan?>">
                            <?php echo form_error('nama_jabatan','<div class="text-small text-danger"></div>')?>
                        </div>
                        <div class="form-group">
                            <label>Gaji Pokok</label>
                            <input type="number" name="gaji_pokok" class="form-control" value="<?php echo $j->gaji_pokok?>">
                            <?php echo form_error('gaji_pokok','<div class="text-small text-danger"></div>')?>
                        </div>
                        <div class="form-group">
                            <label>Uang Transportasi</label>
                            <input type="number" name="uang_transport" class="form-control" value="<?php echo $j->uang_transport?>">
                            <?php echo form_error('uang_transport','<div class="text-small text-danger"></div>')?>
                        </div>
                        <div class="form-group">
                            <label>BPJS</label>
                            <input type="number" name="Bpjs" class="form-control" value="<?php echo $j->Bpjs?>">
                            <?php echo form_error('Bpjs','<div class="text-small text-danger"></div>')?>
                        </div>
                        <div class="form-group">
                            <label>Jumlah Alpha</label>
                            <input type="number" name="jml_potongan" class="form-control" value="<?php echo $j->jml_potongan?>">
                            <?php echo form_error('Bpjs','<div class="text-small text-danger"></div>')?>
                        </div>
                        <div class="form-group">
                            <label>Jumlah Lembur</label>
                            <input type="number" name="jml_tambahan" class="form-control" value="<?php echo $j->jml_tambahan?>">
                            <?php echo form_error('Bpjs','<div class="text-small text-danger"></div>')?>
                        </div>
                        <button type="submit" class="btn btn-success">update</button>
                        </form>
                            <?php endforeach; ?>
                        </div>
                    </div>
                </div>


